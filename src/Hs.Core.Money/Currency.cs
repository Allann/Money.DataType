...
[assembly: System.Runtime.CompilerServices.InternalsVisibleTo("Hs.Core.Money.Tests")]
[assembly: CLSCompliant(true)]

namespace Hs.Core.Money
{
    /// <summary>A unit of exchange; the currency of an <see cref="Amount" /> instance.</summary>
    ...
    {
        private const string DefaultNamespace = "ISO-4217";
        ...
        internal Currency(string code, string @namespace = DefaultNamespace)
        {
        ...
        /// <exception cref="ArgumentNullException">code or number or englishName or symbol is null.</exception>
        /// <exception cref="ArgumentOutOfRangeException">DecimalDigits of code must be greater or equal to zero.</exception>
        internal Currency(string code, string? number, double decimalDigits, string? englishName, string? symbol, string @namespace = DefaultNamespace, DateTime? validTo = null, DateTime? validFrom = null)
            : this()
        {
            ...
            if (decimalDigits < 0 && decimalDigits != DefaultCurrencies.NotApplicable)
            {
        ...
        public static Currency CurrentCurrency
        {
            get
            {
                ...
                return !ci.IsNeutralCulture
                    ? FromCode(new RegionInfo(ci.Name).ISOCurrencySymbol, DefaultNamespace)
        ...
        public decimal MinorUnit 
            => DecimalDigits == DefaultCurrencies.NotApplicable ? MajorUnit : new decimal(1 / Math.Pow(10, DecimalDigits));
        ...
        {
            if (!Registry.TryGet(code, @namespace, out var currency))
            {
        ...
        public static Currency FromRegion(RegionInfo region)
        {
            ...
            return FromCode(region.ISOCurrencySymbol, DefaultNamespace);
        }
        ...
        /// country/region. See also <seealso cref="RegionInfo(string)"/>.</para>
        /// </param>
        ...
        {
            return string.IsNullOrWhiteSpace(name) ? throw new ArgumentNullException(nameof(name)) : FromRegion(new RegionInfo(name));
        }
